function [x,y] = ROFL1OpticalFlow(K,dimsU,tol,sequence,v,alpha,gamma,varargin)
    nPx = prod(dimsU);
    ...
    while err>tol && iterations < maxInnerIt
        ...
        if (mod(iterations,100)==1)
            reverseStr = printToCmd( reverseStr,sprintf('ROF-L1 Optical Flow\nIteration: #%d : Residual %f\n',iterations,err) );
        end
		...
		y3 = max(-gamma,min(gamma,y3 + sigma*(2*Kx3-Kx3Old)));
